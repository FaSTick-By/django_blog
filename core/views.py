...
class SearchResults(HomeView):
    def get_queryset(self):
        ...
        return get_updated_time(qs.filter(title__iregex=query))
...
def author_articles(request, username):
    ...
    total_likes = sum([Like.objects.filter(article=article).count() for article in articles])
    total_dislikes = sum([Dislike.objects.filter(article=article).count() for article in articles])
    context = {
        'user': user,
